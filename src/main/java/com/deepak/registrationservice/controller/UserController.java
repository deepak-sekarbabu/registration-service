...
public class UserController {
    ...
    public Flux<User> getAllUsers(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size) {
        ...
        return this.userRepository.findAllBy(PageRequest.of(page, size)).doOnError(error -> LOGGER.error("Error retrieving users: {}", error.getMessage()));
    ...
    public Mono<User> createUser(@Valid @RequestBody User user) {
        ...
        return this.userRepository.save(user).doOnSuccess(createdUser -> LOGGER.info("User Id : {} has been created", createdUser.getId())).onErrorResume(e -> {
